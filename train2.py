...
import torch
from model.utils import update_ds_batch,all_key_set_init,get_double_ds,get_ds
...
#                   num_workers=4, n_epochs=20, print_every=1 if 'SEARCH_OUTPUT_FP' not in os.environ else 100,
...
#     ds, is_ordered_key = update_ds_batch(model,ds,is_ordered_key,max_type_id,pipe)
    
#     print(f"第{orderlearing_num}轮order训练，{x} -> {len(is_ordered_key)} max: {stop_num}")

# ds = get_double_ds(ds, pipe)
ds = DataSet()
ds = ds.load("/disk1/wxl/Desktop/DeepKE/example/baseline/BARTNER/caches/ds_copy.pt")
print("开始正式训练！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！")
trainer = Trainer(train_data=ds, model=model, optimizer=optimizer,
                  loss=Seq2SeqLoss(),
                  batch_size=batch_size, sampler=sampler, drop_last=False, update_every=1,
