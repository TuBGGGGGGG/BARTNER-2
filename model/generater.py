...
class SequenceGeneratorModel_One_Dec(nn.Module):
    """
    用于封装Seq2SeqModel使其可以做生成任务

    """

    def __init__(self, seq2seq_model: Seq2SeqModel, bos_token_id_ner, bos_token_id_re, eos_token_id=None, max_length=30, max_len_a=0.0,
                 num_beams=1, do_sample=True,
                 repetition_penalty=1, length_penalty=1.0, pad_token_id=0,
                 restricter=None):
        """
        ...
        self.seq2seq_model = seq2seq_model
        self.restricter = restricter
        self.generator_ner = SequenceGenerator(seq2seq_model.decoder, max_length=max_length, max_len_a=max_len_a,
                                           num_beams=num_beams,
                                           do_sample=do_sample,
                                           bos_token_id=bos_token_id_ner,
                                           eos_token_id=eos_token_id,
                                           repetition_penalty=repetition_penalty, length_penalty=length_penalty,
                                           pad_token_id=pad_token_id,
                                           restricter=restricter)
        self.generator_re = SequenceGenerator(seq2seq_model.decoder, max_length=max_length, max_len_a=max_len_a,
                                           num_beams=num_beams,
                                           do_sample=do_sample,
                                           bos_token_id=bos_token_id_re,
                                           eos_token_id=eos_token_id,
                                           repetition_penalty=repetition_penalty, length_penalty=length_penalty,
                                           pad_token_id=pad_token_id,
                                           restricter=restricter)
    
    def forward(self, src_tokens, tgt_tokens, re_tgt_tokens, src_seq_len=None, re_tgt_seq_len=None, tgt_seq_len=None, first=None, update_tree=False):
        """
        透传调用seq2seq_model的forward
        ...
        :return:
        """
        return self.seq2seq_model(src_tokens, tgt_tokens, re_tgt_tokens, src_seq_len, re_tgt_seq_len, tgt_seq_len, first, update_tree)
    ...
    def predict(self, src_tokens, src_seq_len=None, first=None):
        ...
        state = self.seq2seq_model.prepare_state(src_tokens, src_seq_len, first)
        state_re = self.seq2seq_model.prepare_state(src_tokens_clone, src_seq_len_clone, first_clone)
        result_ner = self.generator_ner.generate(state_re)
        result_re = self.generator_re.generate(state)
        assert result_ner.shape[0] == result_re.shape[0], f"{result_re.shape}, {result_ner.shape}"
        return {'pred': result_ner, 're_pred': result_re}
    
class SequenceGeneratorModel_RE_NER(nn.Module):
    """
    用于封装Seq2SeqModel使其可以做生成任务
    ...
    def __init__(self, seq2seq_model: Seq2SeqModel, bos_token_id_ner, bos_token_id_re, eos_token_id=None, max_length=30, max_len_a=0.0,
        ...
        self.generator_ner = SequenceGenerator(seq2seq_model.decoder, max_length=max_length, max_len_a=max_len_a,
                                           ...
                                           pad_token_id=pad_token_id,
                                           restricter=restricter)
        self.generator_re = SequenceGenerator(seq2seq_model.decoder_re, max_length=max_length, max_len_a=max_len_a,
                                           num_beams=num_beams,
                                           do_sample=do_sample,
                                           bos_token_id=bos_token_id_re,
                                           eos_token_id=eos_token_id,
