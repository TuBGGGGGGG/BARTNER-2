...
import torch
from model.utils import update_ds_batch,all_key_set_init,get_double_ds
...
params['params'] = [param for name, param in model_tmp.named_parameters() if not ('bart_encoder' in name or 'bart_decoder' in name)]
...
optimizer_tmp = optim.AdamW(parameters)
...
if save_model == 1:
    save_path = f'save_models/{dataset_name}_{seed}'
else:
    save_path = None
validate_every = 100000

max_type_id = len(pipe.mapping2targetid) + 2
all_update_key = all_key_set_init(ds)
is_ordered_key = set()
stop_num = len(all_update_key)
orderlearing_num = 0

while len(is_ordered_key) < stop_num and orderlearing_num < 1:
    orderlearing_num += 1
    trainer = Trainer(train_data=ds, model=model_tmp, optimizer=optimizer_tmp,
                  loss=Seq2SeqLoss(),
                  batch_size=batch_size, sampler=sampler, drop_last=False, update_every=1,
                  num_workers=4, n_epochs=10, print_every=10 if 'SEARCH_OUTPUT_FP' not in os.environ else 100,
                  ...
                  callbacks=callbacks, check_code_level=-1, test_use_tqdm='SEARCH_OUTPUT_FP' not in os.environ,
                  test_sampler=SortedSampler('src_seq_len'), dev_batch_size=batch_size)
    trainer.train(load_best_model=False)

    x = len(is_ordered_key)
    ds, is_ordered_key = update_ds_batch(model_tmp,ds,is_ordered_key,max_type_id,pipe)
    
    print(f"第{orderlearing_num}轮order训练，{x} -> {len(is_ordered_key)} max: {stop_num}")
    
# del model_tmp
# torch.cuda.empty_cache()
ds = get_double_ds(ds, pipe)
ds.save("/disk1/wxl/Desktop/DeepKE/example/baseline/BARTNER/caches/ds_10_easy.pt")
exit()
